...
def main()->int:
    p=argparse.ArgumentParser(description='Injeta falha controlada para auditar o loop em copia descartavel')
    ...
    if a.fault=='wrong-strip':
        if 'return value.strip()' not in text: raise SystemExit('padrao esperado nao encontrado; nenhuma alteracao feita')
        target.write_text(text.replace('return value.strip()','return value'),encoding='utf-8')
    print(f'Falha {a.fault} injetada em {target.relative_to(ROOT)}')
    return 0
